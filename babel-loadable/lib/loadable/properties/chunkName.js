...
function chunkNameProperty({
  ...
  function generateChunkNameNode(callPath, prefix) {
    ...
    if (importArg.isTemplateLiteral()) {
      return prefix ? t.binaryExpression("+", t.stringLiteral(prefix), sanitizeChunkNameTemplateLiteral(combineExpressions(importArg.node))) : t.templateLiteral(importArg.node.quasis.map((quasi, index) => transformQuasi(quasi, index === 0, importArg.node.quasis.length === 1)), importArg.node.expressions);
    }
  ...
  function replaceChunkName(callPath) {
    ...
    let chunkNameNode = generateChunkNameNode(callPath, getChunkNamePrefix(webpackChunkName));
